...
- **搜索查询应具体收窄**：插件向模型注入 `web_search` 查询指引。每条查询应围绕一个明确目标，补充具体实体/主题、事件或指标，以及必要的时间、地区、版本或来源限定；概览类请求应按主题拆分，不应把多个无关问题拼成一个范围过大的查询。插件不静默改写用户查询。
...
- **搜索范围过宽**：智谱 MCP 可能因查询主题过于泛化或范围过大而拒绝请求；插件返回固定短错误 `ZHIPU_CONTENT_FILTERED`：“智谱查询到敏感词拒绝本次输出。搜索范围不要过于泛化，将请求收窄为一个明确的目标，补充具体实体、时间、地区、指标或来源，用客观、精确的查询重试。”不会自动重复请求、扩大范围或回退到其他搜索后端。
...
| `ZHIPU_CONTENT_FILTERED` | 智谱查询到敏感词拒绝本次输出；查询可能过于泛化或范围过大 | 将请求收窄为一个明确目标，补充具体实体、时间、地区、指标或来源，用客观、精确的查询重试 |
