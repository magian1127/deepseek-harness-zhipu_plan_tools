...
const zh = {
  ...
  enabled: '启用智谱工具',
  enabledDesc: '总开关:关闭后搜索/读取后端停用、仓库工具卸载;设置入口保留。',
  search: '联网搜索(接管 web_search)',
  searchDesc: '把内置 web_search 后端替换为智谱联网搜索 MCP。停用后 web_search 报后端不可用;彻底恢复内置需移除挂载行中的 searchProvider。',
  reader: '网页读取(接管 web_fetch)',
  readerDesc: '把内置 web_fetch 后端替换为智谱网页读取 MCP(markdown 正文)。仅在部署启用 web_fetch 后生效;彻底恢复内置需移除挂载行中的 fetchProvider。',
  zread: '开源仓库工具',
...
const en = {
  ...
  enabledDesc: 'Master switch: off disables the search/reader backends and unregisters repo tools; this card stays.',
  search: 'Web search (takes over web_search)',
  searchDesc: 'Replaces the built-in web_search backend with the Zhipu search MCP. When off, web_search reports an unavailable backend; fully restoring the built-in requires removing searchProvider from the mount row.',
  reader: 'Web reader (takes over web_fetch)',
  readerDesc: 'Replaces the built-in web_fetch backend with the Zhipu reader MCP (markdown). Only effective once the deployment enables web_fetch; fully restoring the built-in requires removing fetchProvider from the mount row.',
