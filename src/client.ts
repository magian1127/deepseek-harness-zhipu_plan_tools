...
const zh = {
  ...
  readerDesc: '把内置 web_fetch 后端替换为智谱网页读取 MCP(markdown 正文)。仅在部署启用 web_fetch 后生效;停用后回退受限 HTTP(S) 文本抓取。',
  zread: '开源仓库工具',
...
const en = {
  ...
  readerDesc: 'Replaces the built-in web_fetch backend with the Zhipu reader MCP (markdown). Once web_fetch is enabled, turning this off uses a bounded HTTP(S) text fetch.',
